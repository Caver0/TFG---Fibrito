...
DietMode = Literal["autogenerated", "manual"]
...
def _normalize_diet_mode(value: str | None) -> DietMode:
    if str(value or "").strip() == "manual":
        return "manual"
...
class ManualDietFoodInput(BaseModel):
    food_code: str = Field(min_length=1, max_length=160)
    quantity: float = Field(gt=0)


class ManualDietMealInput(BaseModel):
    meal_number: int = Field(ge=1)
    foods: list[ManualDietFoodInput] = Field(default_factory=list)


class ManualDietCreateRequest(BaseModel):
    meals_count: int = Field(ge=3, le=6)
    meals: list[ManualDietMealInput] = Field(default_factory=list)
    base_diet_id: str | None = None
...
class DietBase(BaseModel):
    diet_mode: DietMode = "autogenerated"
...
def serialize_daily_diet(document: dict[str, Any]) -> DailyDiet:
    ...
    return DailyDiet(
        ...
        diet_mode=_normalize_diet_mode(document.get("diet_mode")),
